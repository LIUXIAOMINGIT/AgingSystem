...
namespace Cmd
{
    ...
    {
        protected List<SinglePumpPackage> m_PumpPackages = new List<SinglePumpPackage>();
    ...
    public class GetC9Alarm : GetAlarm
    {
        public override void SetBytes(byte[] payloadData)
        {
            if (payloadData.Length == 0)
            ...
            for (int iLoop = 0; iLoop < count; iLoop++)
            {
                Array.Clear(temp, 0, packageSize);
                Array.Copy(payloadData, iLoop * packageSize, temp, 0, packageSize);
                m_PumpPackages.Add(new SingleC9PumpPackage(temp));
            }
        }
    }
